export default class BattlePhaseScene extends Phaser.Scene {
  ...
  showCards() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const playerImg = this.add.image(px, bottomY, `card_${player.id}`).setScale(0.44);
      const enemyImg = this.add.image(px, topY, `card_${enemy.id}`).setScale(0.44);
  ...
  runNextRound() {
    ...
    this.time.delayedCall(400, () => {
      if (outcome === 'player') {
        this.playerWins++;
        this.tweens.add({ targets: playerImg, scale: 0.55, x: playerImg.x, duration: 200, yoyo: true });
        this.tweens.add({ targets: enemyImg, scale: 0.3, angle: 10, duration: 200 });
      } else if (outcome === 'enemy') {
        this.enemyWins++;
        this.tweens.add({ targets: enemyImg, scale: 0.55, x: enemyImg.x, duration: 200, yoyo: true });
        this.tweens.add({ targets: playerImg, scale: 0.3, angle: -10, duration: 200 });
      } else {
        this.tweens.add({ targets: [playerImg, enemyImg], scale: 0.4, duration: 200 });
      }
    });
